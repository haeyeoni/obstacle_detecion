...
void pointCloudHandler(const sensor_msgs::PointCloud2::ConstPtr &depthCloudMsg) {
    ...
    if (!depthCloudIn->empty()){   
        ...
        voxelize(depthCloudIn, depthCloudVoxelized, VOXEL_SIZE);
        std::cout << "voxelized cloud size:" <<depthCloudVoxelized->size()<< std::endl;
        
        transformToMap(*depthCloudVoxelized, *depthCloudInMapCoord); // initialized according to the odometry
        std::cout << "voxelized cloud size:" <<depthCloudVoxelized->size()<< std::endl;
...
void imageHandler(const sensor_msgs::ImageConstPtr& image, const sensor_msgs::ImageConstPtr& depth)
{
    ...
    std::cout<< color_ptr->image <<std::endl;
    colorimage = open3d::Image(color_ptr->image);;
    depthimage = open3d::Image(depth_ptr->image);
...
int main(int argc, char **argv)
{
    ...
    //SUBSCRIBER
    message_filters::Subscriber<sensor_msgs::Image> imageSub(nh, "/camera/color/image_raw", 10);
    message_filters::Subscriber<sensor_msgs::Image> depthSub(nh, "/camera/depth/image_rect_raw", 10);
    typedef sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image> MySyncPolicy;
    Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), imageSub, depthSub);
    sync.registerCallback(boost::bind(&imageHandler, _1, _2));
    
    //PUBLISHER
    ros::Publisher pubObstacleCloud = nh.advertise<sensor_msgs::PointCloud2>("obstacle_cloud", 100);
